...
lista_parole_uniche = [w for w in list(valid_string) if
					   len(w.split('_')) <= 1]
lista_espressioni = [w for w in list(valid_string) if
					 len(w.split('_')) > 1]

# vocabolario_index_tripadvisor = json.load(open('../data/output/tripadvisor/vocabolario.json'))
vocabolario_index_twitter = json.load(
	open('data/vocabolario_twitter.json'))
...
def replace_word_index_tripadvisor(x):
	...
	return [vocabolario_index_tripadvisor[w] for w in x if
			w in vocabolario_index_tripadvisor.keys()]
